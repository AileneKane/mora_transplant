...
abamgermdat<-dat2[dat2$SpPlant=="ABAM",]#600 rows
...
#Add microclimate
tshegermdat <- join(tshegermdat, climdatsub, by=c("Stand","Block","Canopy","Understory"), match="first")
...
AICctab(consgermod.tshe,gddgermod.tshe,gstgermod.tshe,lightgermod.tshe,light2germod.tshe,matgermod.tshe,sdgermod.tshe,gddlightgermod.tshe,gstlightgermod.tshe,matlightgermod.tshe,sdlightgermod.tshe)
#
...
#Now climate models
tsmegermdat<-na.omit(tsmegermdat)
tsmegermdat$SeedsAdded<-as.factor(tsmegermdat$SeedsAdded)
...
AICctab(consgermod.tsme,gddgermod.tsme,gstgermod.tsme,lightgermod.tsme,light2germod.tsme,matgermod.tsme,sdgermod.tsme,gddlightgermod.tsme,gstlightgermod.tsme,matlightgermod.tsme,sdlightgermod.tsme)
##ABAM
#add microclimate data
...
nullgermod.abam<-glmmadmb(abamy~1+(1|Block), data=abamgermdat, family="binomial")
nullgermod.abam<-glmmadmb(abamy~1+(1|Block), data=abamgermdat, family="binomial")
...
dat2011$Block<-as.character(dat2011$Block)
colnames(climdatsub3)[7]<-"meanGST"
colnames(dat2011)[2]<-"Stand"
#Ok, so now I have climate data and germination in the same place. 
#separate out species
alldat$snow_cover_duration=as.numeric(alldat$snow_dur)
alldat$GDD_total=as.numeric(alldat$GDD_total)
alldat$meanGST=as.numeric(alldat$meanGST)
alldat$MAT=as.numeric(alldat$MAT)
alldat$Light_GDD=as.numeric(alldat$Light_GDD)
...
abamgermdat<-na.omit(abamgermdat)
abamgermdat$SeedsAdded<-as.factor(abamgermdat$SeedsAdded)
abamgermdat$tsmey<-cbind(abamgermdat$SeedsAdded,abamgermdat$TotalFails)
...
consgermod.tsme<-glmmadmb(tsmey ~ Stand  +Origin + Canopy + Understory  +Stand:Origin+Stand:Canopy+Stand:Understory+Origin:Canopy+Origin:Understory+Canopy:Understory+Stand:Canopy:Understory+(1|Block),family="binomial", data=tsmegermdat)
consgermod.tshe<-glmmadmb(tsmey ~ Stand  +Origin + Canopy + Understory  +Stand:Origin+Stand:Canopy+Stand:Understory+Origin:Canopy+Origin:Understory+Canopy:Understory+Stand:Canopy:Understory+Moss_cent+(1|Block),family="binomial", data=tshegermdat)
